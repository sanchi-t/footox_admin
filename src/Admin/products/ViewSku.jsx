...
export function ViewSku() {
  ...
  const handleClick = (id,col,item,i) =>{
    ...
    if(quant){
        ...
        data.append('unique', aman);
        data.append('Quantity', quant);
        console.log(data.get('productId'))
        

        axios.post('http://localhost:4000/admin4/', data).then(res => {
  ...
  return (
    <>
    <br></br>
      <Heading>Product Id :- {location.state.id}</Heading><br></br>
      ...
      <>
            ...
            <Box mx={"3"} >
                  <Button>
                    <Icon
                          ...
                          onClick={()=>handleClick(id,col,item, i )}
                        />
                    </Button>
        ...
        <br></br>
        <button type="submit" className="btn btn-primary " style={{marginLeft : '1170px'}} onClick={handleSubmit}>Update Details</button>
